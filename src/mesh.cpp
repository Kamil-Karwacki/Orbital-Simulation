...
Mesh CreateIcoSphere(unsigned int depth)
{
    ...
    unsigned int indices[] = {4, 0, 2, 4, 2, 1, 4, 1, 3, 4, 3, 0,
    ...
    for (int v = 0; v < 4; v++)
    {
        for (int i = 0; i < depth + 1; i++)
        {
            ...
            if (v == 3)
                newVertex.position =
                    Interpolate(vertices[3], vertices[0], ratio);
            else
                newVertex.position =
                    Interpolate(vertices[v], vertices[v + 1], ratio);
    ...
    for (int i = 1; i < depth + 1; i++)
    {
        float ratio = (1.0f / (depth + 1)) * i;
        for (int l = 0; l < 4; l++)
        { // for every one of four edges
            glm::vec3 newLayerPos =
                Interpolate(vertices[l], vertices[4], ratio); // current corner
            ...
            for (int j = 0; j < depth - i + 1; j++)
            {
                ...
                Vertex newVertex{};
                newVertex.position =
                    Interpolate(newLayerPos, newLayerPos2,
                                ratio2); // interpolate to the next edge
                mesh.vertices.emplace_back(newVertex);
            }
        }
        if (i == depth)
    ...
    for (int i = 1; i < depth + 1; i++)
    {
        ...
        { // for every one of four edges
            glm::vec3 newLayerPos =
                Interpolate(vertices[l], vertices[5], ratio); // current corner
            glm::vec3 newLayerPos2;                           // next corner
            if (l == 3)
    ...
    int sideLength = 0,
        originalSideLength; // Number of vertices in current side

    int lastIndex = 4 + 4 * (depth - layer); // Last index in
    int cornerCount = 0; // Number of corners program have looped over. Set to 0
                         // at every new layer
    for (int i = 0; i < topTriangleVertices - 1; i++)
    {
        ...
        if (i == lastIndex - 1)
        { // if its last vertex in current layer
            ...
            {
                mesh.indices.emplace_back(i);
                mesh.indices.emplace_back(i - previousLayerIndex + cornerCount +
                                          1);
                mesh.indices.emplace_back(currentLayer);
            }
            layer++;
    ...
    for (int i = 0; i < mesh.vertices.size(); i++)
    {
        ...
        if (i == currentLayer || i + topTriangleVertices == currentLayer)
        { // if its new layer
            sideLength = i + depth + 1 - layer;
            originalSideLength = depth + 1 - layer;
        }

        if (i == sideLength || i + topTriangleVertices == sideLength)
        { // if its corener
            sideLength += originalSideLength;
            cornerCount++;
        }
        ...
        { // if its last vertex in current layer
            ...
            else
            {
                ...
                if (layer != 0)
                {
                    mesh.indices.emplace_back(i);
                    mesh.indices.emplace_back(currentLayer);
                    mesh.indices.emplace_back(previousLayerIndex +
                                              (i - currentLayer) + cornerCount +
                                              1);
                }
        ...
        else
        {
            ...
            {
                mesh.indices.emplace_back(i);
                mesh.indices.emplace_back(topTriangleVertices + i -
                                          cornerCount);
                mesh.indices.emplace_back(i + 1);
                continue;
            }
            else
            {
                ...
                mesh.indices.emplace_back(i + 1);
                mesh.indices.emplace_back(previousLayerIndex +
                                          (i - currentLayer) + cornerCount + 1);
            }

            mesh.indices.emplace_back(i);
            mesh.indices.emplace_back(nextLayer + (i - currentLayer) -
                                      cornerCount);
            mesh.indices.emplace_back(i + 1);
        }
    }
    ...
    std::cout << "Created ico sphere " << depth
              << " vertices: " << mesh.vertices.size()
              << " indices: " << mesh.indices.size() << " " << glfwGetTime()
              << "\n";
