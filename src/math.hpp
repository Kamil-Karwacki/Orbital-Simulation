...
template <typename T> inline T Interpolate(T a, T b, float t)
{
    return a + t * (b - a);
}
...
inline bool MouseRaycast()
{
    ...
    glm::vec4 ray_eye =
        glm::inverse(glm::perspective(glm::radians(fov),
    ...
    for (int i = 0; i < planets.size(); i++)
    {
        ...
        float a = pow(rayEnd.x - rayStart.x, 2) +
                  pow(rayEnd.y - rayStart.y, 2) + pow(rayEnd.z - rayStart.z, 2);
        float b = 2 * ((rayEnd.x - rayStart.x) * (rayStart.x - center.x) +
                       (rayEnd.y - rayStart.y) * (rayStart.y - center.y) +
                       (rayEnd.z - rayStart.z) * (rayStart.z - center.z));
        float c = pow(center.x, 2) + pow(center.y, 2) + pow(center.z, 2) +
                  pow(rayStart.x, 2) + pow(rayStart.y, 2) + pow(rayStart.z, 2) -
                  2 * (center.x * rayStart.x + center.y * rayStart.y +
                       center.z * rayStart.z) -
                  (planets[i].scale.x * planets[i].scale.x);
...
inline double Perlin(double x, double y, double z)
{
    ...
    x2 = Interpolate(Grad(aba, xf, yf - 1, zf), Grad(bba, xf - 1, yf - 1, zf),
                     u);
    y1 = Interpolate(x1, x2, v);

    x1 = Interpolate(Grad(aab, xf, yf, zf - 1), Grad(bab, xf - 1, yf, zf - 1),
                     u);
...
inline double OctavePerlin(double x, double y, double z, int octaves,
                           double persistence)
{
    double total = 0;
    double frequency = 1;
