...
void DrawPlanet(Planet planet, unsigned int shaderProgram)
{
    ...
    glDrawElements(GL_TRIANGLES,
                   static_cast<unsigned int>(planet.mesh.indices.size()),
                   GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
}
...
void UpdatePlanetsVel(Planet &planet1, Planet &planet2)
{
    ...
    glm::vec3 force = (glm::normalize(forceDir) * GRAVITATIONAL_CONST *
                       planet1.mass * planet2.mass) /
                      glm::distance2(planet1.position, planet2.position);
    if (glm::distance(planet1.position, planet2.position) <
        planet1.scale.x + planet2.scale.x)
    {
        force = glm::vec3(0);
    }
...
void UpdatePlanetsVel2(Planet &planet1, Planet &planet2, float timeStep)
{
    ...
    if (glm::distance(planet1.position, planet2.position) >
        planet1.scale.x + planet2.scale.x)
        force = (glm::normalize(forceDir) * GRAVITATIONAL_CONST * planet1.mass *
                 planet2.mass) /
                glm::distance2(planet1.position, planet2.position);
...
void SelectPlanet() { MouseRaycast(); }
...
void UpdateColors(Planet &planet)
{
    for (int i = 0; i < planet.mesh.vertices.size(); i++)
    {
        ...
        {
            if (glm::length(planet.mesh.vertices[i].position) >
                planet.heights[j].height)
            {
                planet.mesh.vertices[i].color = glm::vec3(
                    planet.heights[j].col[0], planet.heights[j].col[1],
                    planet.heights[j].col[2]);
            }
        }
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, planet.mesh.vertices.size() * sizeof(Vertex),
                 &planet.mesh.vertices[0], GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, planet.mesh.EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 planet.mesh.indices.size() * sizeof(unsigned int),
                 &planet.mesh.indices[0], GL_STATIC_DRAW);
...
void ApplyNoise(Planet &planet, Noise noise)
{
    ...
    for (int i = 0; i < planet.mesh.vertices.size(); i++)
    {
        ...
        float noiseValue =
            OctavePerlin(abs(pointer2.x), abs(pointer2.y), abs(pointer2.z),
                         noise.octaves, noise.persistance);
